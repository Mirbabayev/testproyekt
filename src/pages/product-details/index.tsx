...
import { Heart, ShoppingBag, Star, Minus, Plus, ChevronRight, Clock, Gift, Share, SquareInfo, DollarSign, Truck, CircleHelp, Eye } from 'lucide-react';
...
const ProductDetails = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
        ...
        <div>
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button className="parfumbar-btn flex-1 flex items-center justify-center gap-2 py-3 overflow-hidden whitespace-nowrap">
                <ShoppingBag className="w-5 h-5 flex-shrink-0" />
                <span className="truncate">Səbətə at</span>
              </button>
              <button 
                onClick={toggleWishlist}
                className={`parfumbar-btn-outline flex items-center justify-center gap-2 py-3 overflow-hidden whitespace-nowrap ${
                  isInWishlist ? 'bg-primary/5 text-primary' : ''
                }`}
              >
                <Heart className={`w-5 h-5 flex-shrink-0 ${isInWishlist ? 'fill-primary' : ''}`} />
                <span className="truncate">{isInWishlist ? 'Seçilmişlərdədir' : 'Seçilmişlərə əlavə et'}</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Oxşar məhsullar */}
      <section className="my-16 bg-gradient-to-r from-gray-50 to-accent/10 p-8 rounded-xl shadow-sm">
        ...
        {similarProducts.length === 0 && (
          <div className="text-center py-8 text-gray-500">
            Bu məhsula bənzər məhsullar tapılmadı.
          </div>
        )}
      </section>
    </div>
